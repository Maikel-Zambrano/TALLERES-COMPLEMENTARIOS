...
router.get('/', function(req, res, next) {
  ...
  axios.get('http://localhost:8080/v1/pruebas/api/productos').then(respuesta=>{
...
router.get('/producto/modificar/:id', (req,res,next)=>{
  ...
  axios.get(`http://localhost:8080/v1/pruebas/api/productos/${req.params.id}`).then(respuesta=>{
    res.render('productoForm', {producto:  respuesta.data  });
    
  })
})
router.get('/producto/eliminar/:id', (req,res,next)=>{
  axios.delete(`http://localhost:8080/v1/pruebas/api/productos/${req.params.id}`).then(respuesta=>{
    res.redirect('/');
  })
})
